enum Illocution {
  /// The Assertive Illocution
  asr,

  /// The Directive Illocution
  dir,

  /// The Declarative Illocution
  dec,

  /// The Interrogative Illocution
  irg,

  /// The Verificative Illocution
  ver,

  /// The Admonitive Illocution
  adm,

  /// The Potentiative Illocution
  pot,

  /// The Hortative Illocution
  hor,

  /// The Conjectural Illocution
  cnj;

  /// ID for Quaternary top
  String idQuaternary() {
    return "quaternary_illocution_$name";
  }

  /// Path for Quaternary top
  String pathQuaternary() {
    return switch (this) {
      asr =>
        "M 0.00 8.80 Q 5.00 -3.34 10.00 -2.40 L 10.00 -12.40 -7.50 5.10 -6.30 6.30 0.00 0.00 0.00 8.80 Z",
      dir =>
        "M 32.40 -10.05 Q 22.19 -3.33 16.35 -16.35 L 0.00 0.00 0.00 10.45 Q 4.80 0.95 10.00 5.65 L 10.00 -7.70 Q 20.79 5.41 32.40 -10.05 Z",
      dec =>
        "M -12.55 -13.30 Q -18.53 3.46 -6.35 8.65 L 0.00 2.30 0.00 10.40 Q 4.80 0.90 10.00 5.60 L 10.00 -10.05 0.00 0.00 Q -12.70 0.52 -12.55 -13.30 Z",
      irg =>
        "M 0.00 0.00 L 0.00 5.00 Q 4.75 0.45 10.00 3.50 L 10.00 -7.60 17.50 -15.15 Q -3.79 -16.22 -1.95 -25.40 -10.29 -12.14 8.85 -8.80 L 0.00 0.00 Z",
      ver =>
        "M -6.35 8.65 L 0.00 2.30 0.00 5.40 Q 5.00 -1.73 10.00 0.75 L 10.00 -10.00 0.00 0.00 Q -10.80 -13.10 -22.45 2.30 -12.20 -4.35 -6.35 8.65 Z",
      adm =>
        "M 0.00 0.00 L 0.00 10.45 Q 4.80 0.95 10.00 5.65 L 10.00 -7.70 Q 23.79 -6.85 19.40 5.70 32.93 -7.48 16.35 -16.35 L 0.00 0.00 Z",
      pot =>
        "M -10.00 -7.65 Q -0.20 -23.65 17.50 -15.15 L 10.00 -7.60 10.00 3.50 Q 4.75 0.45 0.00 5.00 L 0.00 0.00 8.85 -8.80 Q 1.70 -14.95 -10.00 -7.65 Z",
      hor =>
        "M 10.00 5.00 L 10.00 -10.00 -2.60 -10.00 4.90 -17.50 3.70 -18.70 -15.00 0.00 0.00 0.00 0.00 10.00 Q 3.85 2.25 10.00 5.00 Z",
      cnj =>
        "M 17.50 -15.10 L 16.30 -16.30 0.00 0.00 0.00 7.75 Q 5.00 -1.69 10.00 1.20 L 10.00 -7.60 17.50 -15.10 Z",
    };
  }

  /// ID for Cr root Secondary top
  String idSecondary() {
    return "secondary_illocution_$name";
  }

  /// Path for Cr root Secondary top
  String pathSecondary() {
    return switch (this) {
      asr => "",
      dir => "M -7.50 0.00 L 0.00 7.50 7.50 0.00 0.00 -7.50 -7.50 0.00 Z",
      dec => "M 10.00 5.00 L 20.00 -5.00 -10.00 -5.00 -20.00 5.00 10.00 5.00 Z",
      irg =>
        "M 20.25 -9.78 L 19.10 -11.03 Q 12.55 0.67 4.70 1.22 -3.10 1.72 -12.75 -8.53 L -20.25 -1.03 Q -13.70 11.02 -1.55 8.52 10.70 6.02 20.25 -9.78 Z",
      ver =>
        "M -20.25 9.78 L -19.10 11.03 Q -12.55 -0.67 -4.70 -1.22 3.10 -1.72 12.75 8.53 L 20.25 1.03 Q 13.70 -11.02 1.55 -8.52 -10.70 -6.02 -20.25 9.78 Z",
      adm =>
        "M 7.60 0.00 L -1.20 8.80 0.00 10.00 20.00 -10.00 -10.00 -10.00 -20.00 0.00 7.60 0.00 Z",
      pot =>
        "M -7.60 0.00 L 1.20 -8.80 0.00 -10.00 -20.00 10.00 10.00 10.00 20.00 0.00 -7.60 0.00 Z",
      hor =>
        "M 12.55 -5.40 Q 11.80 -10.70 7.15 -13.80 2.25 -17.10 -5.80 -17.00 L -13.30 -9.50 Q 1.60 -9.10 5.30 -0.95 9.05 7.25 -2.15 15.95 L -0.90 17.10 Q 6.45 11.90 10.00 5.70 13.30 -0.15 12.55 -5.40 Z",
      cnj =>
        "M -12.55 5.40 Q -11.80 10.70 -7.15 13.80 -2.25 17.10 5.80 17.00 L 13.30 9.50 Q -1.60 9.10 -5.30 0.95 -9.05 -7.25 2.15 -15.95 L 0.90 -17.10 Q -6.45 -11.90 -10.00 -5.70 -13.30 0.15 -12.55 5.40 Z",
    };
  }
}
